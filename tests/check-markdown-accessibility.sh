...
while IFS= read -r -d '' file; do
    ...
    # Check if first 30 lines contain a level-one heading (# )
    # Allows for HTML preamble (e.g. <main>) before the heading
    if head -30 "$file" | grep -q "^# "; then
        echo "✅ $file"
        FILES_PASSED=$((FILES_PASSED + 1))
    else
        echo "❌ $file"
        echo "   Expected: A line starting with '# ' (level-one heading) in the first 30 lines"
        EXIT_CODE=1
    fi
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo ""
    echo "To fix: Add a level-one heading (starting with '# ') in the first 30 lines"
