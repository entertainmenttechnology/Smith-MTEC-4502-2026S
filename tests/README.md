...
Checks all markdown files in the repository for valid heading order (no level skips).
...
This test enforces the axe `heading-order` rule: heading levels should only increase by one (e.g., H2 → H3, not H2 → H4).
